...
st.write('Plot 2: Histogram (non-binned) to visualize counts of all features in the dataset. Note that some features have only unique values (all will count=1) and others have biological outliers (extreme values) that will limit the utility of the visualization.')
st.write('Interaction: Use the drop down menu to select any feature of interst.')
