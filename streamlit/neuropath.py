...
def display_zoomable_image(image_path: str):
    image = load_image(image_path)
    if image is None:
        return

    html_content = image_zoom(image_path, mode="scroll", size=(800, 600), keep_aspect_ratio=False, zoom_factor=4.0, increment=0.2)
...
st.write('Dataset description: SEA-AD is a large multicenter effort which uses postmortem human brain tissue (N=84 donors) to study Alzheimers disease. This webpage is dedicated to increasing visibility of one aspect of this effort: quantitative neuropathology. To supplement the Neuropathology Image Viewer available on sea-ad.org which allows for high resolution synced zooming of both the original and masked image, this viewer is intended to introduce the dataset in more detail and then provide useful and interactive quantitative visualizations! ')
...
with col1:
    ...
    display_zoomable_image("images/tissuewithannotation.png")

# anndata image
with col2:
    st.write('Example annotation mask from the same slide to visualize the features that are quantified (plaques in green and microglia in red. Duplex stains allow for co-localization analysis, which assesses the spatial overlap between plaques and microglia which is relevant for AD pathophysiology. Move mouse over image and scroll to zoom in!)')
    display_zoomable_image("images/markupmask.png")

st.title("Interactive Plots")

st.write('Plot 1: visualizing correlations between key features from the quantitative neuropathology dataset. This correlation matrix allows one to assess relationships across multiple variables readily and provides hover-over functionality for details on demand! Three key features were selected for visualization in this context: the number of neurons (NeuN positive cells), the number of amyloid beta plaques (6e10 positive objects), and the number of pTau bearing cells (AT8 positive cells).')

# read in quant neuropath (from sea-ad website)
# Ensure the file path is correct and relative
filepath = os.path.join("data", "MTG_neuropath.csv")
